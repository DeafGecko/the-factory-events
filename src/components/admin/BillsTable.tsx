...
import { DollarSign, CheckCircle, AlertCircle, X } from 'lucide-react';
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#e8e4dc] bg-[#f5f3ef]">
              <th className="text-left py-3 px-4 font-semibold text-[#8a7a6a] text-[10px] uppercase tracking-[0.8px]">Account</th>
              <th className="text-left py-3 px-4 font-semibold text-[#8a7a6a] text-[10px] uppercase tracking-[0.8px]">Client</th>
              <th className="text-left py-3 px-4 font-semibold text-[#8a7a6a] text-[10px] uppercase tracking-[0.8px] hidden md:table-cell">Event</th>
              <th className="text-left py-3 px-4 font-semibold text-[#8a7a6a] text-[10px] uppercase tracking-[0.8px]">Total</th>
              <th className="text-left py-3 px-4 font-semibold text-[#8a7a6a] text-[10px] uppercase tracking-[0.8px]">Paid</th>
              <th className="text-left py-3 px-4 font-semibold text-[#8a7a6a] text-[10px] uppercase tracking-[0.8px]">Balance</th>
          ...
          <tbody>
            {bookings.length === 0 ? (
              <tr>
                <td colSpan={8} className="py-12 text-center text-[#b0a898]">
              ...
              bookings.map((b) => {
                ...
                return (
                  <tr key={b._id} className="border-b border-[#f0ece6] hover:bg-[#faf9f7] transition">
                    <td className="py-3 px-4 font-mono text-xs text-[#1c1c1e]">{b.accountNumber}</td>
                    <td className="py-3 px-4 font-medium text-[#1c1c1e]">{b.clientName}</td>
                    <td className="py-3 px-4 text-[#6a5a4a] hidden md:table-cell">
                      {b.eventDate ? new Date(b.eventDate).toLocaleDateString() : '—'}
                    </td>
                    <td className="py-3 px-4 font-medium text-[#1c1c1e]">${(b.totalPrice || 0).toFixed(2)}</td>
                    <td className="py-3 px-4 text-[#1c1c1e]">${(b.amountPaid || 0).toFixed(2)}</td>
                      ...
                      {recordingId === b._id ? (
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            className="w-24 border border-[#e8e4dc] rounded-md px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-[#1c1c1e]"
                            placeholder="$"
                            disabled={loading}
                          />
                          <button
                          ...
                          <button
                            ...
                            className="p-1 text-[#8a7a6a] hover:text-[#1c1c1e]"
                          >
                            <X className="w-4 h-4" />
                          </button>
                        </div>
          ...
          <tfoot>
            <tr className="bg-[#f5f3ef] border-t-2 border-[#e8e4dc]">
              <td colSpan={3} className="py-3 px-4 font-semibold text-[#1c1c1e]">Totals</td>
              <td className="py-3 px-4 font-semibold text-[#1c1c1e]">${totalRevenue.toFixed(2)}</td>
              <td className="py-3 px-4 font-semibold text-[#1c1c1e]">${totalPaid.toFixed(2)}</td>
